...
### LeafAppearance
```{r LeafAppearance, echo=FALSE, fig.width=4, fig.height=4}
# Function
LeafInitiation <- function(i, phy_1=71.43, phy_7=16.34, i_max=30, a=420){
  ifelse(
    i <= 6, i*phy_1,
      ifelse(
        i <= i_max,
...
d <- data.frame(
  x = s,
  LeafInitiation = LeafInitiation(s),
  LeafExpansion = LeafInitiation(s) + 1/0.01379
)

d <- melt(d, id.vars="x", value.name = "y", variable.name="Process")

# Graph
p <- ggplot(d, aes(x,y, color=Process)) + geom_line() +
  theme_bw() + labs(x="Node", y="LeafAppearance") +
...
$$LeafExpansion_i=LeafInitiation_i + 1/a$$
