...
\begin{figure*}
  \includegraphics[scale=1.5]{files/structure_nodes.pdf}
\end{figure*}


# Inputs
```{r InputsParameters, echo=FALSE, results='asis'}
s <- sunflo$parameter %>% filter(count==1) %>% select(category, label.doc) %>% group_by(category) %>% summarise(n=n())
n_total <- sum(s$n)
```
...
```{r LeafExpansionDuration, echo=FALSE, fig.width=4, fig.height=3.5, fig.margin=TRUE}
# Data
s <- 1:30
d <- data.frame(
  x = s,
...
## PotentialLeafArea
```{r PotentialLeafArea, echo=FALSE, fig.width=4, fig.height=3.5, fig.margin=TRUE}
...
## LAI
$$LAI = SowingDensity \cdot \sum_{i=1}^{LeafNumber} LeafArea_i$$

## RIE
$$RIE = 1 - exp^{(-ExtinctionCoefficient \cdot LAI)}$$
...
$$dCropBiomass = Radiation \cdot 0.48 \cdot RIE \cdot RUE \cdot dt$$
...
```{r ThermalStressRUE, echo=FALSE, fig.width=4, fig.height=3.5, fig.margin=TRUE}
# Function
...
```{r ThermalStressMineralization, echo=FALSE, fig.width=4, fig.height=3.5, fig.margin=TRUE}
# Function
...
```{r RadiationStressExpansion, echo=FALSE, fig.width=4, fig.height=3.5, fig.margin=TRUE}
# Function
