...
$$
  ...
  \begin{array}{ll}
	...
	(i-5) * Phyllotherm_7 + 6*Phyllotherm_1 & \textrm{if $i \leq LeafNumber$}
	\end{array} \right.
$$
...
$$
\begin{split}
LeafExpansionRate_i = (T_m - T_b) * PotentialLeafArea_i * a * \\
...
## LAI
$$LAI = SowingDensity * PlantLeafArea$$
...
ThermalStressRUE = \left\{ 
  ...
  T_m * \frac{1}{T_{ou} - T_c} - \frac{T_c}{T_{ou} - T_c} & \textrm{if $T_{ou} < T_m < T_c$} \\
  0 & \textrm{else}
	\end{array}
  \right.
$$
...
$$NitrogenDemand = NitrogenCritical = NitrogenCriticalContent*CropBiomass$$
