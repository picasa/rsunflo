...
sunflo <- list(
  climate = read_excel(list_parameter, sheet="climate"),
  parameters = read_excel(list_parameter, sheet="parameters"),
  outputs = read_excel(list_parameter, sheet="outputs"),
  indicators = read_excel(list_parameter, sheet="indicators")
)
sunflo$parameters <- mutate(sunflo$parameters, value=as.numeric(gsub(",",".",value)))

```

```{r data_graph, message=FALSE, warning=FALSE, echo=FALSE, eval=FALSE}
graph <- grViz("inst/files/structure_nodes.gv")
```
...
# Abstract
SUNFLO is a process-based model for the sunflower crop which was developped to simulate the grain yield and oil concentration as a function of time, environment (soil and climate), management practice and genetic diversity [@Debaeke2010; @Casadebaig2011; @Lecoeur2011].  

This model is based on a conceptual framework initially proposed by @Monteith1977 and now shared by a large familly of crop models [REF].
In this framework, the crop dry biomass growth rate ($dDM/dt$) is calculated as an ordinary differential equation [EQ1] function of incident photosynthetically active radiation ($PAR$, $MJ.m^{-2}$), light interception efficiency ($1-exp^{-k \cdot LAI}$) and radiation use efficiency ($RUE$, $g.MJ^{-1}$).
The light interception efficiency is based on Beer-Lambert's law as a function of leaf area index ($LAI$) and light extinction coefficient ($k$). The radiation use efficiency concept [@Monteith1994] is used to represent photosynthesis at the crop scale.

[EQ1] $$ dDM/dt = RUE \cdot (1-exp^{-k \cdot LAI}) \cdot PAR $$

Broad scale processes of this framework, the dynamics of $LAI$, photosynthesis ($RUE$) and biomass allocation to grains were split into finer processes (e.g leaf expansion and senescence, response functions to environmental stresses) to reveal genotypic specificity and to allow the emergence of genotype x environment interactions. Globally, the SUNFLO crop model has about 50 equations and 64 parameters (43 plant-related traits and 21 environment-related).
...
$$
  ThermalTime_d=
	\left\{ 
	\begin{array}{ll}
	\int_0^d (T_m - T_b) \times (1 + WaterStressPhenology)  & \textrm{if $T_m > T_b$} \\  
	0 & \textrm{else}
	\end{array} 
	\right.
$$
...
The rate of leaf appearance depends on air temperature and two phyllochrons as preformed lower leaves appear at a lower rate [@Rey2003]

$$
  LeafInitiationTime_i = \left\{ 
  \begin{array}{ll}
	i \times Phyllotherm_1 & \textrm{if $i \leq 6$} \\  
	(i-5) \times Phyllotherm_7 + 6 \times Phyllotherm_1 & \textrm{if $i \leq LeafNumber$}
	\end{array} \right.
$$
...
Thermal time to 50% of final leaf area was defined as a function of the thermal time required for leaf appearance.
...
$$
\begin{split}
LeafExpansionRate_i = (T_m - T_b) \times PotentialLeafArea_i \times a \times \\
...
$$
\begin{split}
LeafSenescenceRate_i = (T_m - T_b) \times LeafArea_i \times a \times \\
...
$$LeafArea_i=\int LeafExpansionRate_i - \int LeafSenescenceRate_i$$

$$PlantLeafArea = \sum_{i=1}^{LeafNumber} LeafArea_i$$
...
## LAI
$$LAI = SowingDensity \times PlantLeafArea$$

## RIE
$$RIE = 1 - exp^{(-ExtinctionCoefficient \times LAI)}$$
...
$$dCropBiomass = Radiation \times 0.48 \times RIE \times RUE \cdot dt$$
...
$$ThermalStressMineralization = \frac{T_c}{1 + (T_c - 1) \times exp^{(-0.119 \times (T_m - T_b))}}$$
...
$$WaterDemand = K_c \times PET \times RIE$$
...
$$Transpiration = WaterDemand \times WaterStressConductance$$

$$Evaporation = (1 - RIE) \times PET \times SoilConductivity$$
...
$$WaterTotal = SoilWaterCapacity \times SoilDensity \times (1 - StoneContent) \times RootingDepth$$
...
$$ WaterStressProcess = -1 + \frac{2}{1 + exp^{(a \times WaterStress)}}$$ 

with $a \in [-15.6;-2.3]$, genotype-dependant response parameter


## WaterStressPhenology
$$WaterStressPhenology = a \times (1 - WaterStressConductance)$$
with $a=0.1$, scaling parameter for water-stress plant heating

## WaterStressMineralization
$$WaterStressMineralization = 1 - (1 - y_0) \times (1 - RelativeWaterContent_{layer1})$$
...
```{r NitrogenDemand, echo=FALSE, fig.width=4, fig.height=4, fig.margin=TRUE}
# Function
NitrogenContent <- function(biomass, a=4.53, b=0.42) {
  pmin(a, a*biomass^-b)
}
...
d <- data_frame(x=biomass) %>%
  mutate(
    Critical = NitrogenContent(x, a=4.53, b=0.42),
    Maximum = NitrogenContent(x, a=6.49, b=0.44)
  ) %>%
  gather(Process, y, -x)

p <- ggplot(d, aes(x, y, color=Process)) + geom_line() + theme_bw() +
  labs(x="CropBiomass (t.ha-1)", y="NitrogenContent (%)") +
  theme(legend.direction = "horizontal", legend.position = "bottom")

print(p)
```

$$ NitrogenContent =	min(a, a \times CropBiomass^{-b})$$

with: 

* $CropBiomass$, Daily shoot biomass ($t.ha^{-1}$); 
* $a=4.53$ and $b=0.42$ defining $NitrogenContentCritical$ [@Debaeke2012a]; 
* $a=6.49$ and $b=0.44$ defining $NitrogenContentMaximum$ [@Debaeke2012a]; 

$$NitrogenDemand = NitrogenCritical = NitrogenCriticalContent \times CropBiomass$$

## NitrogenSupply

$$NitrogenSupply = NitrogenAbsorbed = NitrogenAbsorbedMassFlow + NitrogenAbsorbedTransport$$

$$NitrogenAbsorbedMassFlow = TranspirationRate_l \times SoilNitrogen$$

## NitrogenStress

$$NitrogenStress = NitrogenSupply/NitrogenDemand = NNI $$ Vitesse d'extraction d'eau par les racine dans C1 (mm/j)
...
$$RadiationStressExpansion = s \times a + \frac{b}{1 + exp^{(\frac{c - IPAR/LAI}{d})}}$$ 
